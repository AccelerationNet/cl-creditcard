...
(defmethod initialize-instance :after ((self echeck-data) &key &allow-other-keys)
  (unless (and (stringp (bank-acct-type self))
    ...
    (error 'invalid-echeck-data :message "bad bank-acct-type" :echeck-data self))
  (unless (and (typep (bank-aba-code self) '(string 9))
	       (every #'digit-char-p (bank-aba-code self)))
    (error 'invalid-echeck-data :message "bad bank-aba-code" :echeck-data self))
  (unless (and (length-between-p (bank-acct-num self) 1 20)
	       (every #'digit-char-p (bank-acct-num self)))
    (error 'invalid-echeck-data :message "bad bank-acct-num" :echeck-data self))
  (unless (length-between-p (bank-name self) 0 50)
    (error 'invalid-echeck-data :message "bad bank-name" :echeck-data self))
  (unless (length-between-p (bank-acct-name self) 1 50)
    (error 'invalid-echeck-data :message "bad bank-acct-name" :echeck-data self))
  (unless (and (stringp (echeck-type self))
	       (member (echeck-type self) +echeck-types+ :test #'string=))
    (error 'invalid-echeck-data :message "bad echeck-type" :echeck-data self))
  (unless (and (stringp (echeck-type self))
	       (member (echeck-type self) +supported-echeck-types+ :test #'string=))
    (error 'invalid-echeck-data :message "unsupported echeck-type" :echeck-data self)))
